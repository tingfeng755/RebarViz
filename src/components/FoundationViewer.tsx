      ...
      <group>
        {colRebars.map(rb => (
          <group key={rb.id} onClick={(e) => { e.stopPropagation(); onSelect({
            ...
            calcValue: `${lae_val}mm`, desc: `弯折要求 max(6d, 150)=${bend_mm}mm`, color: 'bg-red-600'
          })}}>
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full h-screen bg-white overflow-hidden">
      <div className="flex-1 relative bg-slate-50">
        <div className="absolute top-6 left-6 z-10"><h1 className="text-xl font-black text-slate-800 italic">DJ 独立基础 3D 实验室</h1></div>
        {selected && (
          <div className="absolute top-6 right-6 z-50 w-72 bg-white rounded-xl shadow-2xl border p-5">
             <div className="flex justify-between items-center mb-4 font-black"><span>{selected.name}</span><button onClick={() => setSelected(null)}>✕</button></div>
             <div className="bg-slate-900 rounded-xl p-4 text-center text-white">
                <p className="text-4xl font-black font-mono">{selected.calcValue}</p>
                <p className="text-[11px] text-blue-400 mt-2">{selected.formula}</p>
             </div>
             <p className="text-[11px] text-slate-500 mt-4 italic">{selected.desc}</p>
          </div>
        )}
      ...
      <div className="w-full lg:w-96 bg-white p-8 shadow-2xl border-l overflow-y-auto">
        <h3 className="font-black text-slate-800 mb-8 border-b pb-4">参数控制中心</h3>
        <div className="space-y-6">
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1"><label className="text-[11px] font-bold text-slate-400">混凝土强度</label><select value={config.concGrade} onChange={e => setConfig({...config, concGrade: e.target.value})} className="w-full p-2 bg-slate-50 rounded-xl text-sm font-bold shadow-sm">{Object.keys(LAB_MAP).map(g => <option key={g} value={g}>{g}</option>)}</select></div>
              <div className="space-y-1"><label className="text-[11px] font-bold text-slate-400">抗震等级</label><select value={config.seismicGrade} onChange={e => setConfig({...config, seismicGrade: e.target.value})} className="w-full p-2 bg-slate-50 rounded-xl text-sm font-bold shadow-sm">{Object.keys(ZETA_AE_MAP).map(g => <option key={g} value={g}>{g}</option>)}</select></div>
            </div>
          </section>
          <section className="space-y-4 pt-4 border-t">
            <p className="text-[10px] font-black text-red-600 uppercase tracking-widest bg-red-50 px-2 py-1 rounded inline-block">2. 柱筋配置 (nx × nz)</p>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="text-[11px] text-slate-400">X 向根数</label><input type="range" min="2" max="10" value={config.nx} onChange={e => setConfig({...config, nx: parseInt(e.target.value)})} className="w-full accent-red-600" /><div className="text-right text-xs font-bold text-red-600">{config.nx} 根</div></div>
              <div><label className="text-[11px] text-slate-400">Z 向根数</label><input type="range" min="2" max="10" value={config.nz} onChange={e => setConfig({...config, nz: parseInt(e.target.value)})} className="w-full accent-red-600" /><div className="text-right text-xs font-bold text-red-600">{config.nz} 根</div></div>
            </div>
            <div className="space-y-1"><label className="text-[11px] font-bold text-slate-400">柱纵筋 Φ (mm)</label><input type="number" value={config.colD} onChange={e => setConfig({...config, colD: Number(e.target.value)})} className="w-full p-2.5 bg-slate-50 rounded-xl text-sm font-bold" /></div>
          </section>
          <section className="space-y-4 pt-4 border-t">
            <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest bg-blue-50 px-2 py-1 rounded inline-block">3. 基础底筋与面筋</p>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1"><label className="text-[11px] font-bold text-slate-400">底筋直径 d (mm)</label><input type="number" value={config.foundD} onChange={e => setConfig({...config, foundD: Number(e.target.value)})} className="w-full p-2.5 bg-slate-50 rounded-xl text-sm font-bold" /></div>
              <div className="space-y-1"><label className="text-[11px] font-bold text-slate-400">底筋间距 S (mm)</label><input type="number" value={config.foundSpacing} onChange={e => setConfig({...config, foundSpacing: Number(e.target.value)})} className="w-full p-2.5 bg-slate-50 rounded-xl text-sm font-bold" /></div>
            </div>
            <div className="flex items-center justify-between p-3 bg-purple-50 rounded-xl">
               <span className="text-[11px] font-bold text-purple-600 uppercase">开启顶部双向面筋</span>
            ...
            {config.hasTopRebar && (
              <div className="grid grid-cols-2 gap-4 animate-in fade-in">
                ...
                <div className="space-y-1"><label className="text-[11px] text-slate-400">面筋间距 S'</label><input type="number" value={config.topSpacing} onChange={e => setConfig({...config, topSpacing: Number(e.target.value)})} className="w-full p-2 bg-purple-50 rounded-lg text-sm" /></div>
              </div>
            )}
          </section>
        </div>
