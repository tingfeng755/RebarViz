...
function FoundationScene({ config, onSelect }) {
  ...
  const meshBottomY = -fH + c + rFound; const meshTopY = -fH + c + fD + rFound; const bottomY = meshTopY + rFound;
  ...
  return (
    <group position={[0, fH/2, 0]}>
      ...
      <group>{foundRebarsX.map((z, i) => (<mesh key={`fx-${i}`} position={[0, meshBottomY, z]} rotation={[0, 0, Math.PI/2]}><cylinderGeometry args={[rFound, rFound, fL - 2*c, 8]} /><meshStandardMaterial color="#2563eb" /></mesh>))}</group>
      <group>
        {colRebarPos.map((pos, i) => {
          const pts = []; pts.push(new THREE.Vector3(pos.x, 1.2, pos.z));
          const arcS = 10; const cx = pos.x + cD*2.5*scale * pos.dx; const cy = bottomY + cD*2.5*scale; const cz = pos.z + cD*2.5*scale * pos.dz;
          for (let j = 0; j <= arcS; j++) {
            const t = (j / arcS) * (Math.PI / 2);
            pts.push(new THREE.Vector3(cx - cD*2.5*scale * pos.dx * Math.cos(t), cy - cD*2.5*scale * Math.sin(t), cz - cD*2.5*scale * pos.dz * Math.cos(t)));
          }
          pts.push(new THREE.Vector3(pos.x + bend_mm*scale * pos.dx, bottomY, pos.z + bend_mm*scale * pos.dz));
          return (
            <group key={`c-${i}`} onClick={() => onSelect({ 
              name: '柱纵向插筋', spec: `Φ${colD}`, formula: `${zeta_ae} × ${lab_factor}d`, calcLabel: 'lae', calcValue: `${lae_val}mm`, color: 'bg-red-600'
            })}>
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full h-screen bg-slate-50 relative overflow-hidden">
      <div className="flex-1 relative bg-slate-100">
          ...
          <div className="absolute top-4 right-4 z-50 w-64 bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden">
            ...
            <div className="p-4 space-y-2 text-sm">
              ...
              <div className="bg-slate-50 p-2 rounded border text-center">
                <div className="text-[10px] text-slate-400">22G101 算式</div>
                <div className="font-mono text-blue-700">{selectedRebar.formula}</div>
        ...
        {mounted && <Canvas camera={{ position: [3, 2, 4] }}><ambientLight /><directionalLight position={[5, 5, 5]} /><FoundationScene config={config} onSelect={setSelectedRebar} /><Grid args={[10, 10]} cellColor="#cbd5e1" /><OrbitControls /></Canvas>}
      </div>
