      ...
      <group>
        {gridX.map((z, i) => (
          <mesh key={`bx-${i}`} position={[0, bY, z]} rotation={[0,0,Math.PI/2]}>
            <cylinderGeometry args={[fD/2, fD/2, L*s-2*c*s, 8]} /><meshStandardMaterial color="#2563eb" />
          </mesh>
        ))}
        {gridZ.map((x, i) => (
          <mesh key={`bz-${x}`} position={[x, tY, 0]} rotation={[Math.PI/2,0,0]}>
            <cylinderGeometry args={[fD/2, fD/2, B*s-2*c*s, 8]} /><meshStandardMaterial color="#3b82f6" />
          </mesh>
        ))}
      </group>
        ...
        <div className="space-y-8">
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1"><label className="text-[11px] font-bold text-blue-600">底筋直径 d</label><input type="number" value={cfg.foundD} onChange={e=>setCfg({...cfg,foundD:Number(e.target.value)})} className="w-full p-2 bg-blue-50 border border-blue-100 rounded-xl text-xs font-mono font-bold text-blue-600" /></div>
            </div>
          </section>
