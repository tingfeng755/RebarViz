...
export function Sidebar() {
  ...
  return (
    <>
      {hidden && (
        <button onClick={() => setMode('expanded')} className="hidden md:flex fixed left-0 top-1/2 z-40 w-6 h-12 bg-white border border-gray-200 rounded-r-lg shadow-sm items-center justify-center text-gray-400 cursor-pointer"><PanelLeftOpen className="w-4 h-4" /></button>
      )}
      ...
      <aside className={`hidden md:flex flex-col shrink-0 border-r border-gray-200 bg-gray-50 transition-all duration-200 ${hidden ? 'w-0 border-r-0' : collapsed ? 'w-16' : 'w-52'}`}>
        <nav className="flex-1 py-3 px-2 space-y-1">
          {NAV.map(({ href, label, desc, icon: Icon }) => {
            ...
            return (
              <Link key={href} href={href} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors cursor-pointer ${active ? 'bg-accent/10 text-accent border border-accent/20' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800'}`}>
